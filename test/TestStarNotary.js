...
it('lets 2 users exchange stars', async () => {
  ...
  let user1 = accounts[1];
  let user2 = accounts[2];
  await instance.createStar('John Hopkins', starId1, { from: user1 });
  await instance.createStar('Oprah', starId2, { from: user2 });
...
it('lets a user transfer a star', async () => {
  ...
  // 3. Verify the star owner changed.let instance = await StarNotary.deployed();
  let instance = await StarNotary.deployed();
  let starId = 9;
  let user1 = accounts[1];
  let user2 = accounts[2];
  await instance.createStar('Zendaya', starId, { from: user1 });
...
it('lookUptokenIdToStarInfo test', async () => {
  ...
  let starId = 10;
  let user1 = accounts[1];
  await instance.createStar('Udacistar', starId, { from: user1 });
  const starName = await instance.lookUptokenIdToStarInfo.call(starId);
  assert.equal(starName, 'Udacistar');
});
